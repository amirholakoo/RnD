...
def main_loop():
    ...
    while True:
        ...
        try:
            ...
            try:
                response = requests.get(f"{THERMAL_API_URL}/?sensor_id=1", params=params)
                data = response.json()
                value = data["temperature"]
                payload = f"t{int(float(value) + 0.5)}.0"

                size_press_temp = requests.get(f"{THERMAL_API_URL}/?sensor_id=2", params=params)
                size_press_temp = size_press_temp.json()
                size_press_temp = int(size_press_temp["temperature"])
            except:
                continue
            ...
            if response:
                ...
                if response.startswith("n=") and "=" in response:
                    ...
                    data["SizePressTemp"] = size_press_temp
                    print(data)
                    ...
                    PLC_Keys.objects.bulk_create([PLC_Keys(key=key,value=str(type(value).__name__))for key in missing_keys])
