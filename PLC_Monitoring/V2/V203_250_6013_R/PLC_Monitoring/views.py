...
def get_historical_chart_data(request):
    ...
    time_thresholds = {
        ...
        '3-month': now - timedelta(days=30),
        '6-month': now - timedelta(days=60),
        '9-month': now - timedelta(days=90)
    }
    ...
    try:
        ...
        #logs = PLC_Logs.objects.all().order_by('CreationDateTime')
        logs = PLC_Logs.objects.filter(
            roll__plc_id=plc_id,
            roll__Is_Deleted=False,
            roll__CreationDateTime__gte=threshold,
            is_running=True
        ).only("CreationDateTime", "json_data")
        # Collect all unique keys
        all_keys = set()
        def add_to_all_key(key):
            all_keys.add(key)
