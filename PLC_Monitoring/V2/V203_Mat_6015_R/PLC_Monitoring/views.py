        ...
        while True:
            ...
            if not health:
                health = TCP_CONNECTION(
                    ServerLastUpdate=time.time(),
                    ClientLastUpdate=time.time(),
                )
                health.save()
...
def export_logs_csv(request):
    ...
    now = timezone.now()
    from_time = False
    filename=f"PLC-MAT-MAIN-history-4h-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    ...
    if time_range:
        ...
        if from_time:
            ...
            filename=f"PLC-MAT-MAIN-history-{time_range}-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    # ---- Date parsing ----
    if from_date and to_date:
        try:
            ...
            filename=f"PLC-MAT-MAIN-history-from-{from_date}-to-{to_date}.csv"
        except:
