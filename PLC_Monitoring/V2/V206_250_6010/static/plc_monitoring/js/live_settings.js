...
function updateCardStyle(card, key, value) {
    ...
    if (key === 'st') {
        ...
        statusElement = document.getElementById("machine_status");
        if (value === '1') {
            card.style.background = 'rgba(76, 175, 80, 0.2)';
            card.style.borderColor = '#4caf50';
            statusElement.classList.add("on");
            statusElement.classList.remove("off");
            statusElement.children[1].innerHTML = "START";
        } else {
            statusElement.classList.add("off");
            statusElement.classList.remove("on");
            statusElement.children[1].innerHTML = "STOP";
        }
        ...
        if (config.min_value != null && numValue < config.min_value) {
            alertColor = config.color_min || '#ffaa00';
