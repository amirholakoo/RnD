...
def export_logs_csv(request):
    ...
    now = timezone.now()
    from_time = False
    filename=f"PLC-STOCK-history-4h-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    ...
    if time_range:
        ...
        if from_time:
            ...
            filename=f"PLC-STOCK-history-{time_range}-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    # ---- Date parsing ----
    if from_date and to_date:
        try:
            ...
            filename=f"PLC-STOCK-history-from-{from_date}-to-{to_date}.csv"
        except:
