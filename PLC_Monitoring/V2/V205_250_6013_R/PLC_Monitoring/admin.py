...
from .models import PLC, PLC_Logs, PLC_Keys, Rolls, Roll_Breaks, ChartExcludedKeys, KeyAlertConfig, LiveSettingsGroup
import jdatetime
...
class JalaliDateTimeMixin:
    ...
    def jalali_creation_datetime(self, obj):
        if obj.CreationDateTime:
            jdt = jdatetime.datetime.fromgregorian(datetime=obj.CreationDateTime)
    ...
    def jalali_last_update(self, obj):
        if obj.LastUpdate:
            jdt = jdatetime.datetime.fromgregorian(datetime=obj.LastUpdate)
            return jdt.strftime('%Y/%m/%d %H:%M:%S')
        return '-'
...
@admin.register(LiveSettingsGroup)
class LiveSettingsGroupAdmin(admin.ModelAdmin):
...
class PLCLogsAdmin(JalaliDateTimeMixin, admin.ModelAdmin):
    list_display = ('id', 'roll', 'is_running', 'plc', 'json_data', 'jalali_creation_datetime', 'jalali_last_update')
...
class PLCKeysAdmin(JalaliDateTimeMixin, admin.ModelAdmin):
    list_display = ('id', 'name', 'fa_name', 'key', 'value', 'order_index', 'group', 'order_in_group', 'live_background', 'value_max', 'description', 'jalali_creation_datetime', 'jalali_last_update')
    search_fields = ('name', 'fa_name', 'key', 'value')
    list_editable = ('order_in_group','order_index','group','live_background', 'value_max')
    ordering = ('-CreationDateTime',)
...
@admin.register(KeyAlertConfig)
class KeyAlertConfigAdmin(JalaliDateTimeMixin, admin.ModelAdmin):
    list_display = (
        'id', 'key',
        'min_value', 'min_value_2', 'max_value', 'max_value_2',
        'color_max', 'color_max_2', 'color_min', 'color_min_2',
        'alert_types', 'jalali_creation_datetime', 'jalali_last_update'
    )
    search_fields = ('key',)
    ordering = ('-CreationDateTime',)