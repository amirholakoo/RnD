...
def get_plc_keys(request):
    """Get all PLC keys with group info"""
    keys = PLC_Keys.objects.select_related('group').all()
    ...
    for key in keys:
        ...
        if key.group_id:
            item['group_id'] = key.group_id
            item['group_name'] = key.group.name
            item['group_order'] = key.group.order
            item['order_in_group'] = key.order_in_group
        else:
            item['group_id'] = None
            item['group_name'] = None
            item['order_in_group'] = 0
        data.append(item)
...
def live_settings(request):
    ...
    setting_json = json.dumps(plc.setting or {}, ensure_ascii=False) if plc and getattr(plc, 'setting', None) else '{}'
...
def get_historical_chart_data(request):
    ...
    try:
        # Get logs directly from PLC_Logs by plc_id and time range (no roll dependency)
        # Include all logs regardless of is_running status for chart data
...
def sse_live_chart(request):
    """
    Stream only changed chart points via SSE.
    """
    pass
    # def event_stream():
    #     previous_values = {}
    ...
    #             if ChartExcludedKeys.objects.filter(key=key).exists():
    #                 continue
    ...
    #             name = plc_key.fa_name or plc_key.name or plc_key.key
    #             current_value = str(value)
    #             old_value = previous_values.get(name)

    #             if old_value != current_value:
    #                 previous_values[name] = current_value
...
def update_keys_order_bulk(request):
    """Bulk update order_index for multiple PLC keys (for drag & drop). Keys in same group get min index of that group."""
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            group_min_order = {}
            for pk in PLC_Keys.objects.filter(group_id__isnull=False):
                gid = pk.group_id
                if gid not in group_min_order or pk.order_index < group_min_order[gid]:
                    group_min_order[gid] = pk.order_index
            for gid, min_ord in group_min_order.items():
                PLC_Keys.objects.filter(group_id=gid).update(order_index=min_ord)
...
def group_create(request):
    """Create a group from two or more keys (by key string). Optionally set name."""
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Method not allowed'}, status=405)
    try:
        key_ids_json = request.POST.get('key_ids')
        name = (request.POST.get('name') or '').strip() or 'گروه جدید'
        if not key_ids_json:
            return JsonResponse({'status': 'error', 'message': 'key_ids الزامی است'})
        try:
            key_ids = json.loads(key_ids_json)
        except (ValueError, TypeError):
            return JsonResponse({'status': 'error', 'message': 'فرمت key_ids نامعتبر است'})
        if not isinstance(key_ids, list) or len(key_ids) < 2:
            return JsonResponse({'status': 'error', 'message': 'حداقل دو کلید برای گروه لازم است'})
        keys = list(PLC_Keys.objects.filter(key__in=key_ids).order_by('order_index', 'order_in_group'))
        if len(keys) < 2:
            return JsonResponse({'status': 'error', 'message': 'حداقل دو کلید یافت نشد'})
        key_by_key = {k.key: k for k in keys}
        ordered = [key_by_key[k] for k in key_ids if k in key_by_key]
        if len(ordered) < 2:
            ordered = keys[:2]
        with transaction.atomic():
            grp = LiveSettingsGroup.objects.create(name=name)
            min_order = min(k.order_index for k in ordered)
            for i, pk in enumerate(ordered):
                pk.group_id = grp.id
                pk.order_in_group = i
                pk.order_index = min_order
                pk.save()
        return JsonResponse({'status': 'ok', 'data': {'group_id': grp.id, 'group_name': grp.name}})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)})


@csrf_exempt
def group_add_key(request):
    """Add a key to an existing group."""
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Method not allowed'}, status=405)
    try:
        group_id = request.POST.get('group_id')
        key_id = request.POST.get('key')  # key string (e.g. 'mi1')
        if not group_id or not key_id:
            return JsonResponse({'status': 'error', 'message': 'group_id و key الزامی است'})
        grp = LiveSettingsGroup.objects.get(id=group_id)
        plc_key = PLC_Keys.objects.get(key=key_id)
        if plc_key.group_id == grp.id:
            return JsonResponse({'status': 'ok', 'message': 'کلید قبلاً در این گروه است'})
        max_in_group = PLC_Keys.objects.filter(group_id=group_id).aggregate(models.Max('order_in_group'))['order_in_group__max'] or -1
        with transaction.atomic():
            plc_key.group_id = grp.id
            plc_key.order_in_group = max_in_group + 1
            plc_key.order_index = PLC_Keys.objects.filter(group_id=group_id).values_list('order_index', flat=True).first() or plc_key.order_index
            plc_key.save()
        return JsonResponse({'status': 'ok', 'message': 'کلید به گروه اضافه شد'})
    except LiveSettingsGroup.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': 'گروه یافت نشد'})
    except PLC_Keys.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': 'کلید یافت نشد'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)})
...
def group_remove_key(request):
    ...
    try:
        ...
        if not key_id:
            return JsonResponse({'status': 'error', 'message': 'key الزامی است'})
        plc_key = PLC_Keys.objects.get(key=key_id)
        if not plc_key.group_id:
            return JsonResponse({'status': 'ok', 'message': 'کلید در گروهی نیست'})
        with transaction.atomic():
            plc_key.group_id = None
            plc_key.order_in_group = 0
            plc_key.save()
        return JsonResponse({'status': 'ok', 'message': 'کلید از گروه خارج شد'})
    except PLC_Keys.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': 'کلید یافت نشد'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)})
...
def group_update_name(request):
    ...
    try:
        ...
        if not group_id:
            return JsonResponse({'status': 'error', 'message': 'group_id الزامی است'})
        if not name:
            return JsonResponse({'status': 'error', 'message': 'name الزامی است'})
        grp = LiveSettingsGroup.objects.get(id=group_id)
        grp.name = name
        grp.save()
        return JsonResponse({'status': 'ok', 'data': {'group_name': grp.name}})
    except LiveSettingsGroup.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': 'گروه یافت نشد'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)})
...
def export_logs_csv(request):
    ...
    now = timezone.now()
    from_time = False
    filename=f"PLC-250-MAIN-history-4h-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    ...
    if time_range:
        ...
        if from_time:
            ...
            filename=f"PLC-250-MAIN-history-{time_range}-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    # ---- Date parsing ----
    if from_date and to_date:
        try:
            ...
            filename=f"PLC-250-MAIN-history-from-{from_date}-to-{to_date}.csv"
        except:
    ...
    all_names = list(PLC_Keys.objects.values_list('name', flat=True).order_by("order_index"))
    all_keys = list(PLC_Keys.objects.values_list('key', flat=True).order_by("order_index"))
    ...
    def stream():
        # header
        headers = ['Row', 'Machine', 'RollNumber', 'DateTime'] + all_names
        ...
        for log in logs:
            ...
            current_state.update(json_data)
            warmup_count += 1
            if not ready:

                if len(current_state) == len(all_keys):
                    ready = True

                elif warmup_count >= warmup_limit:
                    ready = True
            ...
            for i, key in enumerate(all_keys):
                if key in json_data:
                    current_group['row'][4 + i] = current_state[key]
