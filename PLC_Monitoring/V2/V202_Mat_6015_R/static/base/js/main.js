...
// let popupDisplayCounter = 0;
// const MAX_DELAY_MULTIPLIER = 5;

// function showUpdatePopup(versionData) {
//     if (versionCheckTimeoutId) {
//         clearTimeout(versionCheckTimeoutId);
//     }

//     if (!document.getElementById('update-popup')) {
//         console.log(versionData,versionData.desc)
//         const popupHtml = `
//             <div id="update-popup">
//                 <h2>نسخه جدید موجود است! (v${versionData.version})</h2>
//                 <p>${versionData.desc || 'لطفاً برای دریافت آخرین تغییرات، صفحه را بروزرسانی کنید.'}</p>
//                 <button id="btn-update">بروزرسانی</button>
//                 <button id="btn-later">بعداً</button>
//             </div>
//         `;
...
//     const popupElement = document.getElementById('update-popup');
//     const overlayElement = document.querySelector('body.popup-active::before');

//     document.getElementById('btn-update').onclick = () => {
//         updateVersionCookieAndReload(versionData.version);
//         location.reload();
//     };

//     document.getElementById('btn-later').onclick = () => {
//         popupDisplayCounter++;
//         let delay = 30000 // * Math.min(popupDisplayCounter, MAX_DELAY_MULTIPLIER);

//         closePopup();
//         versionCheckTimeoutId = setTimeout(checkVersion, delay);
//     };
// }
...
//     // توجه: overlay به صورت CSS مدیریت می‌شود و با کلاس body.popup-active::before ظاهر و حذف می‌شود
// }

// function updateVersionCookieAndReload(newVersion) {
//     document.cookie = `version=${newVersion}; path=/; expires=Fri, 31 Dec 2099 23:59:59 GMT; SameSite=Strict`;
//     closePopup();
// }
...
//             AM_alert({text:"ارتباط با سرور برقرار نشد!",status:"warning"})
//             popupDisplayCounter = 0;
//             versionCheckTimeoutId = setTimeout(checkVersion, 10000);
//             return; 
//         }

//         const serverVersionData = await response.json();
//         const currentVersion = serverVersionData.version;
//         const description = serverVersionData.desc;

//         const cookies = document.cookie.split('; ');
//         let clientVersion = null;
//         for (const cookie of cookies) {
//             if (cookie.startsWith('version=')) {
//                 clientVersion = cookie.substring('version='.length, cookie.length);
//                 break;
//             }
//         }
//         if (!clientVersion) {
//             updateVersionCookieAndReload(currentVersion);
//             clientVersion = currentVersion;
//         }
        
//         if (currentVersion !== clientVersion) {
            
//             showUpdatePopup({ version: currentVersion, desc: description });
//             // refresh page if not user doing anything
//             versionCheckTimeoutId = setTimeout(() => {
//                 updateVersionCookieAndReload(currentVersion);
//                 location.reload();
//             }, 30000);
//         } else {
//             popupDisplayCounter = 0;
//             versionCheckTimeoutId = setTimeout(checkVersion, 15000); 
//         }
