...
def get_historical_chart_data(request):
    ...
    plc_id = request.GET.get('plc')
    time_range = request.GET.get('range', '4h')
    ...
    if request.method == "POST":
        ...
        if roll_from_request:
            try:
                ...
                last_roll = Rolls.objects.get(roll_number=roll_number)
                rolls = Rolls.objects.filter(CreationDateTime__gte=last_roll.CreationDateTime)
            except (ValueError, TypeError):
...
def generate_csv_rows(logs, all_keys):
    # header
    headers = ['row', 'PM', 'roll_number', 'date'] + all_keys
    yield headers
...
def export_logs_csv(request):
    ...
    now = timezone.now()
    from_time = False
    filename=f"PLC-250-history-4h-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    ...
    if time_range:
        ...
        if from_time:
            ...
            filename=f"PLC-250-history-{time_range}-from-{jdatetime.datetime.now().strftime('%Y-%m-%d')}.csv"
    # ---- Date parsing ----
    if from_date and to_date:
        try:
            ...
            filename=f"PLC-250-history-from-{from_date}-to-{to_date}.csv"
        except:
