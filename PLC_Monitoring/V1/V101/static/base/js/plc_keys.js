...
function renderSettingsTable(keys) {
    ...
    tbody.innerHTML = keys.map(key => `
        <tr>
            ...
            <td>
                <div class="action-btns">
                    <button type="button" class="action-btn edit" onclick="editKey(${key.id}, ${key.plc_id}, '${escapeStr(key.key)}', '${escapeStr(key.value || '')}')">ویرایش</button>
...
// Edit key - fill form
function editKey(id, plcId, key, value) {
    document.getElementById('key_id').value = id;
    document.getElementById('plc_select').value = plcId;
