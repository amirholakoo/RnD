...
def get_historical_chart_data(request):
    ...
    try:
        ...
        for log in logs:
            ...
            interval_ts = int(log.CreationDateTime // interval) * interval * 1000
            for key in all_keys:
        ...
        for roll in rolls:
            ...
            if first_log and first_log.CreationDateTime:
                roll_annotations.append({
                    'x': int(first_log.CreationDateTime * 1000),
        ...
        for rb in roll_breaks:
            if rb.CreationDateTime:
                break_annotations.append({
                    'x': int(rb.CreationDateTime * 1000),
        ...
        for log in all_logs:
            ...
            if not log['is_running'] and range_start is None:
                range_start = int(log['CreationDateTime'] * 1000)
            elif log['is_running'] and range_start is not None:
                range_end = int(log['CreationDateTime'] * 1000)
...
def export_all_logs_xlsx(request):
    ...
    just_run = request.GET.get('just_run')
    ...
    if from_date and to_date:
        ...
        try:
            ...
            threshold_start = jdt_from.togregorian().timestamp()
            threshold_end = jdt_to.togregorian().timestamp()
    ...
    logs = logs_query.select_related('plc', 'roll').filter.order_by('CreationDateTime')
    ...
    for log in logs:
        ...
        interval_ts = int(log.CreationDateTime // interval) * interval
    ...
    for row_idx, (group_key, group_info) in enumerate(sorted_groups, 2):
        ...
        dt_str = jdatetime.datetime.fromtimestamp(ts).strftime('%Y/%m/%d %H:%M:%S') if ts else ''
