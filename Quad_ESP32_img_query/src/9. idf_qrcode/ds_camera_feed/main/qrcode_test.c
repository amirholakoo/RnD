...
// Server URL
#define SERVER_URL "http://192.168.2.29:5000"

// Frame and buffer configuration
#define FRAME_WIDTH 1280
#define FRAME_HEIGHT 720
#define IMAGE_SIZE (FRAME_WIDTH * FRAME_HEIGHT) // 921,600 bytes for grayscale
#define NUM_BUFFERS 1 // Adjustable; max 2 with current settings due to 2MB PSRAM

static uint8_t* buffers[NUM_BUFFERS];
static size_t image_sizes[NUM_BUFFERS];
static int next_capture = 0;
...
static const char 
    ...
    *format_fail = "Non-grayscale data not supported",
...
static void get_image_task(void* pvParameters) {
    while (true) {
        ...
        next_capture = (next_capture + 1) % NUM_BUFFERS;
...
static void image_send_task(void* pvParameters) {
    while (true) {
        ...
        next_send = (next_send + 1) % NUM_BUFFERS;
...
void app_main(void) {
    ...
    for (int i = 0; i < NUM_BUFFERS; i++) {
        buffers[i] = (uint8_t*)heap_caps_malloc(IMAGE_SIZE, MALLOC_CAP_SPIRAM);
        if (!buffers[i]) {
            ESP_LOGE("APP", "Failed to allocate PSRAM buffer %d", i);
            return;
        }
    }
    ...
    camera_config_t config = {
        ...
        .jpeg_quality = 5, // Ignored for grayscale, but kept for compatibility
        .fb_count = 1,
    ...
    // Create semaphores
    empty_sem = xSemaphoreCreateCounting(NUM_BUFFERS, NUM_BUFFERS);
    full_sem = xSemaphoreCreateCounting(NUM_BUFFERS, 0);
