...
# Function to log QR code data
def log_qr_code(device_id, qr_data):
    timestamp = datetime.now().strftime("%Y-%m-d %H:%M:%S")
    with open(qr_log_file, 'a') as f:
        f.write(f"{timestamp} - Device: {device_id}, QR Code: {qr_data}\n")
    print(f"QR Code detected from {device_id}: {qr_data}")
...
@app.route('/send_qr_data', methods=['POST'])
def send_qr_data():
    """Receive and log QR code data from ESP32 devices."""
    device_id = request.headers.get('X-Device-ID', 'Unknown')
    qr_data = request.data.decode('utf-8')
    log_qr_code(device_id, qr_data)
    return "QR data received", 200
