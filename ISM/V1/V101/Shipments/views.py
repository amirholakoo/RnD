...
def shipments(request):
    shipments = Shipment.objects.filter(Is_Deleted=False).order_by("-CreationDateTime")
    context = {
        'shipments': shipments,
    }
    ...
    if request.method == "POST":
        try:
            ...
            if request.POST.get("unit"):
                shipment.unit = Unit.objects.get(id=int(request.POST.get("unit")))
            else:
                shipment.unit = None
