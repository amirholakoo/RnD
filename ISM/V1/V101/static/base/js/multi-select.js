...
function init(element) {
  ...
  function addPlaceholder(wrapper) {
    ...
    if (!tokens.length && !(document.activeElement === input_search))
      input_search.setAttribute(
        "placeholder",
        "جستجو کنید و یا از لیست انتخاب کنید"
      );
  }
  ...
  function createInitialTokens(select) {
    ...
    const selected_options = select.querySelectorAll("option:checked");
    for (let i = 0; i < selected_options.length; i++) {
      const option = selected_options[i];
      // Create a li element similar to how it's done in populateAutocompleteList
      const li = document.createElement("li");
      li.innerText = option.text || option.innerHTML;
  ...
  function createToken(wrapper, value) {
    console.log('__________ss',value);
    const search = wrapper.querySelector(".search-container");
    // Create token wrapper
    const token = document.createElement("div");
    token.classList.add("selected-wrapper");
    const token_span = document.createElement("span");
    const input_html = `<input type="number" value="${value.dataset.value}" class="d-none" name="vision">`
