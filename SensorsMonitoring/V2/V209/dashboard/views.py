...
def chart_info_json(request,sensor_id):
    if request.method == 'POST':
        ...
        if date_range_start and date_range_end:
            ...
            print("date_range_start",date_range_start,"date_range_end",date_range_end)
                                ...
                                if chart_data[index]["time"] + offset < float(x.CreationDateTime):
                                    if not diff > offset:
                                        ...
                                        for i in range(1,int(diff/offset)):
                                            ...
                                            chart_data[index]["timestamps"].append(str(jdatetime.datetime.fromgregorian(datetime=datetime.datetime.fromtimestamp(float(chart_data[index]["time"] + offset*i))).strftime(" %d %b - %H:%M")))
                                            chart_data[index]["time"] = float(chart_data[index]["time"] + offset*i)
                                            chart_data[index]["s"].append(float(chart_data[index]["time"] + offset*i))
        ...
        if has_ai and chart_data and not filter_time == 3600:
            ...
            for x in chart_data[0]["data"]:
                if ai_index < ai_data.count():
                    ...
                    print("difference__________",difference)
                    if same_hour: #(difference <= 1800 and difference >= 0) or (difference >= -1800 and difference <= 0):
