...
EVENT_LOG = "temp/events_log.jsonl"
...
def check_event_log():
    ...
    while True:
        ...
        # Check condition: 
        # 1. File size changed AND (sent_events.json is not empty OR doesn't exist), OR
        ...
        ) or (len(sent_events) == 0 and new_size > 0)  # Resend all if sent_events is empty
        ...
        if should_process:  # file appended and sent_events.json allows processing, or need to resend all
            ...
            elif current_sent_events_status["exists"] and current_sent_events_status["empty"]:
                print("Skipping processing: sent_events.json is empty")
