...
def forklift_panel(request):
    context = {
        'unloads': Unload.objects.all()
    }
    return render(request, 'forklift/forklift_panel.html', context)

def get_vision_detections():
    """Read VisionData from database and count class_name occurrences (excluding forklift)."""
    class_counts = {}
    
    try:
        # Get all VisionData objects
        ...
        for vision_data in vision_data_list:
            ...
            # Get class_name from event
            class_name = vision_data.name
            
            # Skip forklift class_name
            if class_name:
                class_counts[class_name] = class_counts.get(class_name, 0) + 1    
    except Exception as e:
        print(f"Error reading VisionData from database: {e}")
        import traceback
        traceback.print_exc()
        return []
    
    # Convert to list of dicts: [{name: "akhal", count: 5}, ...]
    vision_detections = [{"name": name, "count": count} for name, count in sorted(class_counts.items())]
...
def unload(request):
    ...
    # Get vision detections from events_log.jsonl
