        ...
        try:
            ...
            if license_numbers:
                ...
                for license_number in license_numbers:
                    truck = Truck.objects.filter(License_plate_number=license_number).last()
                    if not truck:
                    ...
                    print("check..")
                    shipment = Shipment.objects.filter(Is_Deleted=False).filter(Truck=truck).filter(CreationDateTime__gte=time.time()-4000).last()
                    print(shipment)
                    if not shipment:
