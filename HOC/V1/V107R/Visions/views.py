...
def organizing_vision_detections(vision_data):
    ...
    def check_for_shipment(organized_data):
        ...
        if not organized_data.class_name == "forklift":
            ...
            #     if is_same_as(organized_data.class_name, unit_name) or (not shipment.unit and organized_data.Type == 5):
            #         organized_data.shipment = shipment
            #         print("shipment found and saved")
            last_shipments = Shipment.objects.filter(Q(CreationDateTime__gte=organized_data.start_time - 6300) & Q(CreationDateTime__lte=organized_data.start_time + 6300))   
            if last_shipments:
                ...
                for shipment in last_shipments:
                    ...
                    unit_name = '' if not shipment.unit else shipment.unit.name
                    print(f"================ {unit_name} =================")
                    if is_same_as(organized_data.class_name, unit_name) or (not shipment.unit and organized_data.class_name == "paper-roll" and organized_data.Type == 5)
                        and not (OrganizingVisionData.objects.filter(Type=1,shipment=shipment,start_time__gte=time.time() - 900,start_time__lte=organized_data.start_time + 3600).exists()):
                        print(f"================ is same =================")
