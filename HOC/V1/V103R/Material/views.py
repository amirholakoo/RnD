...
def add_material_type(request):
    ...
    if request.method == "POST":
        try:
            ...
            material_type.save()
            messages.success(request, 'نوع material با موفقیت افزوده شد')
            return redirect('material_types')
        except Exception as e:
            messages.error(request, 'خطا در افزودن نوع material: '+str(e))
...
def edit_material_type(request, material_type_id):
    ...
    if request.method == "POST":
        try:
            ...
            material_type.save()
            messages.success(request, 'نوع material با موفقیت ویرایش شد')
            return redirect('material_types')
        except Exception as e:
            messages.error(request, 'خطا در ویرایش نوع material: '+str(e))
...
def delete_material_type(request, material_type_id):
    ...
    if request.method == "POST":
        try:
            ...
            material_type.save()
            messages.success(request, 'نوع material با موفقیت حذف شد')
            return redirect('material_types')
        except Exception as e:
            messages.error(request, 'خطا در حذف نوع material: '+str(e))
            return redirect('delete_material_type', material_type_id=material_type_id)
    return redirect('material_types')
