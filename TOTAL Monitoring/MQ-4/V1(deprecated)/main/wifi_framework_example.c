...
/* MQ-4 Sensor Configuration - CHANGE ADC CHANNEL AS NEEDED */
#define MQ4_ADC_CHANNEL ADC_CHANNEL_0  // Change this to your ADC channel
...
static void data_sender_task(void* pvParameters)
{
    ...
    ESP_LOGI(DATA_Q_TAG, "MQ-4 sensor initialized on ADC1_CH%d", MQ4_ADC_CHANNEL);
    ...
    while (1) {
        ...
        if (ret == ESP_OK && reading.is_valid) {
            ...
            if (reading.voltage < 0.0f || reading.voltage > 3.3f || 
                ...
                ESP_LOGW(DATA_Q_TAG, "Expected: Voltage 0-3.3V, Resistance 1Ω-1MΩ, PPM 0-10000");
...
void app_main(void)
{
    ...
    ESP_LOGI(WIFI_TAG, "  ADC Channel: %d", MQ4_ADC_CHANNEL);
