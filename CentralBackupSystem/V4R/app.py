            ...
            if os.path.isdir(device_dir):
                ...
                for f in sorted(os.listdir(device_dir), reverse=True):
                    ...
                    if os.path.isfile(full):
                        ...
                        lower = f.lower()
                        if lower.endswith('.sqlite') or lower.endswith('.sqlite3'):
                            btype = 'sqlite'
...
def download_backup(device, filename):
    """Download a specific backup file"""
    try:
        # Determine the correct backup base directory
        external_path = get_external_storage_path()
        
        if external_path:
            # External storage available - backups are in external storage
            from backup import EXTERNAL_BACKUP_DIR
            backup_base = os.path.join(external_path, EXTERNAL_BACKUP_DIR)
        else:
            # No external storage - backups are in local folder
            backup_base = LOCAL_BACKUP_BASE
        
        # Construct the file path
        file_path = os.path.join(backup_base, device, filename)
        
        # Security check: ensure the path doesn't escape the backup directory
        real_backup_base = os.path.realpath(backup_base)
        real_file_path = os.path.realpath(file_path)
        
        if not real_file_path.startswith(real_backup_base):
            logging.warning(f"Attempted directory traversal attack: {device}/{filename}")
            abort(403)  # Forbidden
        
        # Check if file exists
        if not os.path.exists(file_path) or not os.path.isfile(file_path):
            logging.warning(f"Requested file not found: {device}/{filename}")
            abort(404)  # Not found
        ...
        logging.error(f"Error downloading backup {device}/{filename}: {e}")
