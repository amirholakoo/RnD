...
def backup_postgresql():
        ...
        for host_entry in pg_hosts_config.values(): # iterate over host entries
            ...
            for db_data in databases:
                ...
                # db_pass is used as an environment variable for pg_dump, not directly in subprocess call here
                ...
                # Backup directory for this host
                host_name_for_dir = host.replace('.', '_') # Host name for backup folder
                ...
                # pg_dump command using PGPASSWORD environment variable
                # The actual pg_dump command will run on the remote server via SSH
                cmd = f'PGPASSWORD="{db_pass}" pg_dump -U "{db_user}" -h localhost "{db_name}"'

                print(f"Backing up PostgreSQL DB: {db_name} on {host} with user {db_user}")

                try:
                    # Open the file in binary write mode before running the command
                    with open(filepath, "wb") as f:
                        process = subprocess.run(
                            ...
                            stdout=f,  # Use the opened file object here
                    ...
                    if process.returncode != 0:
                        ...
                        print(f"Error backing up {db_name} on {host}: {error_message}")
                        # Clean up empty file if backup failed
                        if os.path.exists(filepath) and os.path.getsize(filepath) == 0:
                            os.remove(filepath)
                    else:
                        print(f"Successfully backed up {db_name} to {filepath}")

                except subprocess.TimeoutExpired:
                    print(f"Error: Backup timed out for {db_name} on {host}")
                    # Clean up empty file if timeout occurred
                    if os.path.exists(filepath) and os.path.getsize(filepath) == 0:
                        os.remove(filepath)
                except FileNotFoundError as e:
                    print(f"Error: SSH command not found. Ensure SSH client is installed and in PATH. Details: {e}")
                    if os.path.exists(filepath) and os.path.getsize(filepath) == 0:
                        os.remove(filepath)
                except Exception as e:
                    print(f"An unexpected error occurred during backup of {db_name} on {host}: {e}")
                    if os.path.exists(filepath) and os.path.getsize(filepath) == 0:
                        os.remove(filepath)
