...
esp_err_t data_sender_send_mq4_data(const char* device_id, const char* sensor_type, float ppm_methane, float voltage, float resistance, uint64_t timestamp)
{
    if (!is_initialized || device_id == NULL || sensor_type == NULL) {
        ESP_LOGE(TAG, "Data sender not initialized or invalid parameters");
        return ESP_ERR_INVALID_STATE;
    ...
    cJSON_AddNumberToObject(data_obj, "ppm_methane", ppm_methane);
    cJSON_AddNumberToObject(data_obj, "voltage", voltage);
    cJSON_AddNumberToObject(data_obj, "resistance", resistance);
    cJSON_AddNumberToObject(data_obj, "timestamp", (int64_t)timestamp);
    cJSON_AddStringToObject(data_obj, "type", "mq4_data");
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "MQ-4 data sent successfully, HTTP status: %d", response.status_code);
        ...
        ESP_LOGE(TAG, "Failed to send MQ-4 data");
    }
